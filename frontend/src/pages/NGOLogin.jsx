...
export default function NGOLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await orgLogin(formData.email, formData.password);
      if (result.success) {
        navigate("/");
      } else {
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-16">
        ...
        <div className="bg-white/90 backdrop-blur-md p-8 rounded-2xl shadow-xl w-full max-w-md">
          ...
          <p className="text-gray-600 text-center text-sm mb-6">
            Welcome back! Manage your campaigns and impact.
          </p>
          ...
          <p className="mt-4 text-center text-sm text-gray-600">
            ...
            <a href="/ngo-signup" className="text-rose-600 hover:underline">
              Sign up here
            </a>
          </p>
          <p className="mt-1 text-center text-sm text-gray-600">
            Are you a Donor?{" "}
            <a href="/donor-Login" className="text-rose-600 hover:underline">
              Donor Login
            </a>
          </p>
        </div>
