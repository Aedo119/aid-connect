...
const mockCampaigns = [
  ...
  },
  {
    ...
    description:
      "Building wells and water filtration systems to provide clean drinking water.",
    progress: 60,
    raised: 45000,
    goal: 75000,
    donors: 189,
  ...
  },
  {
    ...
    description:
      "Providing school supplies, books, and educational resources to children in need.",
    progress: 74,
    raised: 18500,
    goal: 25000,
    donors: 156,
  ...
  },
  {
    ...
    description:
      "Setting up mobile clinics to provide essential healthcare services.",
    progress: 100,
    raised: 60000,
    goal: 60000,
    donors: 300,
...
const mockDonations = [
  ...
  },
  {
    ...
    type: "money",
    date: "2025-08-10",
    campaign: "Educational Support",
  },
  {
    id: 7,
    donor: "Local School",
    items: "Books, Notebooks",
    type: "clothes",
    date: "2025-08-09",
    campaign: "Educational Support",
  },
...
export default function NGODashboard() {
  ...
  const generateAnalytics = (campaignsData, donationsData) => {
    ...
    donationsData.forEach((donation) => {
      donationTypeCount[donation.type] =
        (donationTypeCount[donation.type] || 0) + 1;
    });
    ...
    const donationTypes = Object.keys(donationTypeCount).map((type) => ({
      ...
      percentage: Math.round(
        (donationTypeCount[type] / donationsData.length) * 100
      ),
    }));
    ...
    const donorDistribution = [
      {
        range: "$1-50",
        count: monetaryDonations.filter((d) => d.amount <= 50).length,
      },
      {
        range: "$51-100",
        count: monetaryDonations.filter((d) => d.amount > 50 && d.amount <= 100)
          .length,
      },
      {
        range: "$101-250",
        count: monetaryDonations.filter(
          (d) => d.amount > 100 && d.amount <= 250
        ).length,
      },
      {
        range: "$251+",
        count: monetaryDonations.filter((d) => d.amount > 250).length,
      },
    ];
  ...
  const exportReports = () => {
    ...
    donations.forEach((d) => {
      csv += `"${d.donor}",${d.type},"${d.amount || d.items}",${d.date},"${
        d.campaign
      }"\n`;
    });
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-800">
                    {summary.total_active_campaigns}
                  </p>
                </div>
              </div>
            </div>
